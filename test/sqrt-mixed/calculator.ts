...
describe("验证 Bonding Curve Swap 计算函数 算子测试", async () => {
    ...
    let px = Ether.div(100);
    let tvl = Ether.mul(1e6).mul(2).div(3);
    describe("Sqrt Mixed Bonding Swap", async () => {
        it("Sqrt 验证 计算方程 的 结果", async () => {
            ...
            let testOne = async (nativeAsset) => {
                ...
                console.log(ethers.utils.formatEther(ans.daoTokenAmount), ethers.utils.formatEther(supply))
                expect(
