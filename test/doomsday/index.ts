...
describe("验证 Doomsday 销毁功能", async () => {
    describe("Exp Mixed Hotpot Token", async () => {
        it("校验 unpause 情况下 doomsday 功能", async () => {
            ...
            await expect(hotpotTokenAbi.connect(treasury).declareDoomsday(), "非 platform 用户不可 declareDoomsday").to
                .reverted;

            await expect(hotpotTokenAbi.connect(treasury).destroyForDoomsday(), "treasury 用户在 declare 之前不可 destory").to
                .reverted;
            ...
            await expect(hotpotTokenAbi.connect(buyer).destroyForDoomsday(), "非 treasury 用户不可 destoryForDoomsday").to
                .reverted;
            await expect(hotpotTokenAbi.connect(platform).destroyForDoomsday(), "非 treasury 用户不可 destoryForDoomsday").to
                .reverted;
            ...
            expect(asset.eq(balanceOfTreasuryAfter.add(gas).sub(balanceOfTreasuryBefore)), "合约余额 未打入 treasury 地址").to
                .true;
        });
    });
});
