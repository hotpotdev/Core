    ...
    it("部署 HotpotToken & Governor", async () => {
        const deploy = async () => {
            ...
            let setBalance = async (signer: SignerWithAddress, number: BigNumber) => {
                await network.provider.send("hardhat_setBalance", [signer.address, Ether.mul(number)._hex.replace(/0x0+/, "0x")]);
            }
            ...
            let mintAndDelegate = async (signer: SignerWithAddress, number: BigNumber) => {
                ...
                await setBalance(signer, need.nativeTokenPaidAmount.mul(2))
                let tx = await tokenAbi.connect(signer).mint(await signer.getAddress(), 0, { value: need.nativeTokenPaidAmount })
                await tx.wait()
        ...
        describe('Governor Proposal Control Hotpot Method', () => {
            it('Governor Set Metadata', async () => {
                ...
                tx = await governorAbi.connect(buyer2).propose(
                    [tokenAbi.address], [0], 
                    ["setMetadata(string)"], [ethers.utils.defaultAbiCoder.encode(["string"], [testMetadata])], 
                    "test set metadata"
                );
                await tx.wait()
            ...
            it('Governor Transfer Admin', async () => {
                ...
                tx = await governorAbi.connect(buyer2).propose(
                    [tokenAbi.address], [0],
                    ["setProjectAdmin(address)"], [ethers.utils.defaultAbiCoder.encode(["address"], [buyer2.address])],
                    "test transfer admin"
                );
                await tx.wait()
                let proposalId = await governorAbi.latestProposalIds(buyer2.address)
            ...
            it('Governor Transfer Project Treasury', async () => {
                ...
                tx = await governorAbi.connect(buyer2).propose(
                    [tokenAbi.address], [0],
                    ["setProjectTreasury(address)"], [ethers.utils.defaultAbiCoder.encode(["address"], [buyer2.address])],
            ...
            it('Governor Update Contract Implement', async () => {
                ...
                tx = await governorAbi.connect(buyer2).propose(
                    [factoryAbi.address], [0],
                    ["rejectUpgrade(address,string)"], [ethers.utils.defaultAbiCoder.encode(["address",'string'], [tokenAbi.address,"test"])],
                    "test reject upgrade"
                );
                await tx.wait()
                let proposalId = await governorAbi.latestProposalIds(buyer2.address)
                ...
                await expect(
                    factoryAbi.connect(platform).upgradeTokenImplement(tokenAbi.address),
                    "拒绝后升级操作不能执行"
                ).to.reverted;
