...
describe("验证 Pausable 暂停功能", async () => {
    ...
    describe('Exp Mixed Hotpot Token', async () => {
        it("校验 pause 功能", async () => {
            ...
            await expect(factoryAbi.connect(treasury).pause(hotpotTokenAbi.address),'非 platform 用户不可调用 pause 功能').to.reverted
            await expect(factoryAbi.connect(treasury).unpause(hotpotTokenAbi.address),'非 platform 用户不可调用 unpause 功能').to.reverted
            await expect(factoryAbi.connect(buyer).pause(hotpotTokenAbi.address),'非 platform 用户不可调用 pause 功能').to.reverted
            await expect(factoryAbi.connect(buyer).unpause(hotpotTokenAbi.address),'非 platform 用户不可调用 unpause 功能').to.reverted
            
            await expect(hotpotTokenAbi.connect(treasury).pause(),'any 用户 不可直接调用 factory 的 pause 功能').to.reverted
            await expect(hotpotTokenAbi.connect(treasury).unpause(),'any 用户 不可直接调用 factory 的 unpause 功能').to.reverted
            await expect(hotpotTokenAbi.connect(platform).pause(),'any 用户 不可直接调用 factory 的 pause 功能').to.reverted
            await expect(hotpotTokenAbi.connect(platform).unpause(),'any 用户 不可直接调用 factory 的 unpause 功能').to.reverted
            ...
            await expect(hotpotTokenAbi.connect(buyer).transfer(treasury.address,'100'),'取消暂停后 可以转账').to.ok
