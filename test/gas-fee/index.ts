...
import { HotpotFactory__factory } from "../../typechain/factories/HotpotFactory__factory"
...
describe("验证 Gas Fee 的收取", async () => {
    ...
    describe('Exp Mixed Hotpot Token', async () => {
        ...
        it("校验 mint 与 burn 方法 平台手续费", async () => {
            ...
            console.log('mintRate (', mintRate, '/ 10000 ) mint 消耗 eth 1', 'platform 获得 eth', ethers.utils.formatEther(balance7))
            ...
            console.log('burnRate (', burnRate, '/ 10000 ) 合约 burn 消耗eth', ethers.utils.formatEther(asset8.sub(asset9)), 'platform 获得 eth', ethers.utils.formatEther(balance10))
            expect(asset8.sub(asset9).mul(burnRate).div(10000).eq(balance10), '平台 burn 手续费 set后 计算错误').to.true
            
            let setTax2 = await factoryAbi.setPlatformTaxRate(100,100)
            await setTax2.wait()
        })
