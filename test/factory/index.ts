...
describe("Factory Function Test", async () => {
    it("合约 升级 测试", async () => {
        ...
        await expect(factoryAbi.connect(platform).initialize(platform.address,platform.address), 'Facotry 不可 init 两次').to.reverted
        ...
        await expect(tokenProxy.connect(anyone)["initialize(string,string,address,address,uint256,uint256,bytes)"]
            ("TET2", "TET2", treasury.address, treasury.address, 2000, 2000, data2), "Token 不可以 init 两次").to.reverted
