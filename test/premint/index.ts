    ...
    describe("Mixed Hotpot Token", async () => {
        it("校验 premint 功能", async () => {
            ...
            await expect(
                ...
                "非 Premint 用户不可 进行预铸造"
            ).to.reverted;
            await expect(
                hotpotTokenAbi.connect(premintRole).mint(buyer.address, 0, { value: Ether.mul(10) }),
                "非 Premint 用户不可 进行预铸造"
            ).to.reverted;
            await expect(
                hotpotTokenAbi.connect(treasury).mint(treasury.address, 0, { value: Ether.mul(10) }),
                " treasury 用户默认可以 预铸造"
            ).not.reverted;

            let tx1 = await hotpotTokenAbi
                .connect(treasury)
                .grantRole(await hotpotTokenAbi.PREMINT_ROLE(), premintRole.address);
            await tx1.wait();
            await expect(
                hotpotTokenAbi.connect(buyer).mint(buyer.address, 0, { value: Ether.mul(10) }),
                "非 Premint 用户不可 进行预铸造"
            ...
            let tx2 = await hotpotTokenAbi
                .connect(treasury)
                .revokeRole(await hotpotTokenAbi.PREMINT_ROLE(), premintRole.address);
            await tx2.wait();
            await expect(
                hotpotTokenAbi.connect(buyer).mint(buyer.address, 0, { value: Ether.mul(10) }),
                "非 Premint 用户不可 进行预铸造"
